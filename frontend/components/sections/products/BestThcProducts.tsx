...
function getThcScore(item: ProductListItem) {
  const values: string[] = [];

  const contentThc = item.content?.facts?.thcLevel;
  if (contentThc) values.push(contentThc);

  Object.entries(item.filters ?? {}).forEach(([key, value]) => {
    const normalized = key.toLowerCase();
    if (normalized.includes("thc")) {
      values.push(value);
    }
  });
  ...
  return (
    <section className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-[130px]">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-3xl font-semibold text-pr_w">Best THC</h2>
