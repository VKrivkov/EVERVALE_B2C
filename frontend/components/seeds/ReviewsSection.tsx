...
export default function ReviewsSection({ productId }: ReviewsSectionProps) {
  ...
  const canWriteReview = isAuthenticated || disableAuth;
  const hasReviews =
    derivedSummary.reviewCount > 0 || total > 0 || reviews.length > 0;
  const shouldShowEmptyState = !loading && !hasReviews;
  ...
  return (
    <>
      {shouldShowEmptyState ? (
        <div className="mt-10 rounded-2xl bg-pr_w p-6 text-pr_dg">
          ...
          <button
            ...
            className="mt-6 rounded-full bg-sr_dg px-6 py-2 text-sm font-semibold text-pr_w disabled:opacity-60"
          >
            {formLoading
              ? "Submitting..."
              : canWriteReview
